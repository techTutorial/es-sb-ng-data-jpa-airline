...
create table USER_SCHEMA2.EMPLOYEE_ENTITY (
   ...
   FOREIGN KEY (ID_USER_EMP) REFERENCES USER_SCHEMA2.USER_ENTITY (ID_USER),
...
DROP TABLE IF EXISTS USER_SCHEMA2.ES_EMPLOYEE_CONTRACT;

create table USER_SCHEMA2.ES_EMPLOYEE_CONTRACT (
   ID_ECON BIGINT AUTO_INCREMENT PRIMARY KEY,
   ID_EMP_ECON BIGINT,
   --ID_EMP_CON BIGINT references EMPLOYEE_ENTITY,
   EDATE_ECON varchar(100),
   --END_DATE_ECON DATE,
   --CONSTRAINT ECON_PK PRIMARY KEY (ID_ECON)
   FOREIGN KEY (ID_EMP_ECON) REFERENCES USER_SCHEMA2.EMPLOYEE_ENTITY (ID_EMP)
);
